...
static void errmsg_curl(const char* url, const char* msg, CURLcode code) {
    cerr << msg << " failed with code " << curl_easy_strerror(code)
              << " for url " << url << endl;
}


static void errmsg_curlm(const char* msg, CURLMcode code) {
    cerr << msg << " failed with code " << curl_multi_strerror(code) << endl;
}
...
public:
    static AsyncDownloader* create() {
        ...
        if (multi_handle == NULL) {
            cerr << "curl_multi_init falied" << endl;
            return NULL;
        }
    ...
    void addJob(const char* url) {
        ...
        if (easy_handle == NULL) {
            cerr << "addJob: curl_easy_init failed for url " << url << endl;
            exit(1);
        }
...
void event_cbev(evutil_socket_t socket_fd, short kind, void* userp) {
    ...
    do {
        ...
        if (m && (m->msg == CURLMSG_DONE)) {
            ...
            cout << "[DOWNLOADED] " << finished_job->file_size
                      << " bytes from " << finished_job->url << endl;
...
void timer_cbcurl(CURLM* multi_handle, long timeout_ms, void* userp) {
    ...
    if (timeout_ms > 0) {
        ...
        if (async_downloader->evtimeout == NULL) {
            ...
            if (ev == NULL) {
                cerr << "falied to create timeout event" << endl;
                exit(1);
            }
        ...
        if (evrv != 0) {
            cerr << "failed to add timeout event" << endl;
        }
